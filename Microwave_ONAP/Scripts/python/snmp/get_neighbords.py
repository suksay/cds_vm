...
old_devices_model = env.get_template('old_devices.json')
...
def update_aai(devices):
    ...
    def create_device(device_id, device_info, isExist=False):
        ...
        if isExist == True :
            device_data = json.loads(old_devices_model.render(device_id=device_id, operational_status="down", resource_version=device_info))
        else :
    ...
    def create_pnf(device_id, device_info, isExist=False):
        ...
        if isExist == True :
            pnf_data = json.loads(old_devices_model.render(device_id=device_id, operational_status="down", resource_version=device_info))
        else :
            if device_info['vendor'] == 'huawei' :
